...
namespace Yugen.Toolkit.Standard.Services
{
    ...
    public sealed class MediatorService
    {
        ...
        /// <param name="propertyEnum">The message to register to</param>
        public void Register(Action<object> callback, string propertyEnum) => _internalList.AddValue(propertyEnum, callback);
        ...
        {
            if (!_internalList.ContainsKey(propertyEnum))
            {
                return;
            }

            //forward the message to all listeners
            foreach (Action<object> callback in _internalList[propertyEnum])
            {
                callback(args);
            }
        }
